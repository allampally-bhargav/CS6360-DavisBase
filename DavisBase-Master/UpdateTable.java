...
public class UpdateTable {
	public static void executeUpdateString(String updateString) {
		System.out.println("UPDATE METHOD");
		System.out.println("Executing the string:\"" + updateString + "\"");
		...
		String[] set = DavisBase.setUpComparisions(setTemp);
	...
	public static void update(String table, String[] cmp, String[] set){
		try{
			...
			for(int i = 0; i < keys.length; i++)
				if(keys[i] == key)
					x = i;
			int offset = Page.fetchCellOffset(file, page, x);
			long loc = Page.fetchCellLoc(file, page, x);
