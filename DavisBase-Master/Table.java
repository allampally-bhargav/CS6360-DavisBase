	...
	public static void filter(RandomAccessFile file, String[] cmp, String[] columnName, Buffer buffer){
		try{
			...
			for(int page = 1; page <= numOfPages; page++){
				...
				if(pageType == 0x0D)
				{
					byte numOfCells = Page.fetchCellNumber(file, page); //accesses file header to get number of cells.
		...
		try{
			...
			for(int page = 1; page <= numPages; page++){
				...
				if(pageType == 0x0D){
					...
					if(keys.length == 0)
						return 0;
					int rm = Page.fetchRightMost(file, page);
		...
		try{
			...
			for(int page = 1; page <= numOfPages; page++){
				...
				byte pageType = file.readByte();
					...
					byte numOfCells = Page.fetchCellNumber(file, page);
